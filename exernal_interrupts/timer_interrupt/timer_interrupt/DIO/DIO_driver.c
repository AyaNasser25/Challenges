...
void DIO_WritePin(uint8 PinNum,uint8 PinValue){
	if(PinNum >= PORTA_START && PinNum <=PORTA_END){
		switch(PinValue){
			case TRUE : Set_Bit(PORTA,PinNum);
			break;
			case FALSE : Clear_Bit(PORTA,PinNum);
			break;
			default :
			break ;
		}
		...
		switch(PinValue){
			case TRUE : Set_Bit(PORTB,(PinNum-PORTB_START));
			break;
			case FALSE : Clear_Bit(PORTB,(PinNum-PORTB_START));
			break;
			default :
			break ;
		}
